            ...
            render: function (mData, type, row) {
                ...
                if (uID == row.oRecipient[0]._id && mData == "Bid") {
                    option = `<td class="text-center">
                            <button class="btn btn-primary btn-peach" id="Status" style="font-size: 14px !important; text-transform: none;" nQty="${row.nQuantity}" name="Accepted" amount="${row.nBidPrice.$numberDecimal}" bidderAddress="${row.oBidder[0].sWalletAddress}" bidderID="${row.oBidder[0]._id}" objId="${row._id}" onclick="toggleStatus($(this))">Accept</button>
                        </td>
                        <td class="text-center">
                        <button class="btn btn-secondary" id="Status" style="font-size: 14px !important; text-transform: none;"  name="Rejected" objId="${row._id}" bidderID="${row.oBidder[0]._id}" bidderAddress="${row.oBidder[0].sWalletAddress}" onclick="toggleStatus($(this))">Reject</button>
                        </td>`;
...
function buyQuantityChange() {
    if ($('#nftQuantity').val() <= 0) {
        ...
        $('#lblQuantityError').html('Must be greater than 0');
...
function bidQuantityChange() {
    if ($('#bidQuantity').val() <= 0) {
        $('#lblBidQuantityError').removeClass("d-none").addClass("d-block")
        $('#lblBidQuantityError').html('Quantity Must be greater than 0');
...
function checkout() {
    ...
    if (oOptions.eBidStatus == "Sold") {
        ...
        oContract.methods.buyNow(nTokenID, sCurrentOwnerAddress, window.sessionStorage.getItem("sWalletAddress"), oOptions.nQuantity)
            .send({
