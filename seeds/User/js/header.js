...
$(() => {
    ...
    if (bIsValidUser === "false") {
        ...
        if (window.sessionStorage.getItem("Authorization") !== null) {
            ...
            window.sessionStorage.removeItem('sWalletAddress');
            setTimeout(() => {
            ...
            if (ee != undefined) {
                ...
                if (web3 && network == 97 && ee != undefined) {
                    ...
                    await web3.eth.getAccounts().then(function (result) {
                        sWalletAddress = (result[0]).toString();
                    })
...
try {
    window.ethereum.on('accountsChanged', function (accounts) {
        if (window.sessionStorage.removeItem('Authorization') != null)
            logout();
        else
            window.location.href = '/';
    });
...
async function signMessage(methodName) {
    if (window.ethereum) {
        ...
        try {
            ...
            const message = `Blockchain Australia uses this cryptographic signature in place of a password, verifying that you are the owner of this Ethereum address - ${timestamp}`;
                    ...
                    if (methodName == "Signin") {
                        $.ajax({
                            ...
                            success: function (result, status, xhr) {
                                ...
                                window.sessionStorage.setItem('sWalletAddress', result.data.sWalletAddress);
...
async function loadRedeemablePoints() {
    web3 = new Web3(web3.currentProvider);
    var sAccount;
    await web3.eth.getAccounts().then(function (aAccounts) {
